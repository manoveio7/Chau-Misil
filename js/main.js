...
{	
	...
	listo = false;
	//vel = 3//1.5;
	mision = parseInt(localStorage.getItem("mision")) || 1; // Aseguramos que sea número
	// La velocidad base es 3, y aumenta 0.2 por cada nivel superado
	vel = 3 + (mision * 0.2); 

	// También es justo aumentar un poco la rotación para que no se sienta "pesado"
	velRotPlayer = 2.5 + (mision * 0.05);
	anguloPlayer = 270;	
	rotaDer = false;
	rotaIzq = false;
	MISILES = [];	
...
//	audioAvion.setDetune(50);
	...
	for(let i = 0; i < 5; i++)
	{
		...
		misil.velRotMisil = (Phaser.Math.Between(10, 20) / 10)// + mision < 3 ? 0 : (mision * 0.1); // Giran más rápido cada nivel
		misil.velMisil = (Phaser.Math.Between(vel * 10, vel * 10 + 20) / 10) //+ mision < 3 ? 0 : (mision * 0.2); // Corren más
		misil.dist = 0;
		misil.seVe = false;
...
function empezarDetener(elemento)
	...
	{
		control = setTimeout(() => {
        // Si el nivel es alto, permite más misiles en pantalla
        let limiteMisiles = 0 //2 + Math.floor(mision / 2); 
		if(mision > 2 && mision < 5)
			limiteMisiles = mision -2
		else 
			limiteMisiles = MISILES.length
        
        if (revisarCantMisilPantalla() < limiteMisiles) {
            agregarMisil();
        }
        console.log('Limites Misil '+limiteMisiles)
        revisarNivelFuel();	
        controlJuego();
        
        // El tiempo de espera se reduce 500ms por nivel (mínimo 2 segundos)
    }, Math.max(2000, (Phaser.Math.Between(5, 10) * 1000) - (mision * 500)));
	}
	var rcmp = 0
	function revisarCantMisilPantalla()
	{
		...
		rcmp = misilAct
		console.log('cantidad de Misil en pantalla: '+misilAct)
		return misilAct;
	}
	
	function agregarMisil()
	{
		//aqui digo apartir de que mision aparecen los misiles
		if(mision <= 2 || misionCumplida || !listo)
			return;
	...
	function misiones() {
    let m = parseInt(mision);

    if (m <= 3) {
        // --- NIVELES INICIALES (1, 2 y 3) ---
        objectivoEstrellas = m; // Nivel 1 = 1 estrella, Nivel 2 = 2...
        objetivoMisiles = 0;    // Sin requisito de destruir misiles
        vel = 2.5 + (m * 0.1);  // Incremento de velocidad muy suave
    } else {
    ...
    console.log("Misión: " + m + " | Estrellas: " + objectivoEstrellas + " | Misiles: " + objetivoMisiles);
}
